...
void GraphicsApplication::createSurface() {
    sampleMain();
}

void GraphicsApplication::setSize(uint32_t w, uint32_t h) {
    width = w;
    height = h;
}

void GraphicsApplication::drawFrame() {
    // not done yet
...
void GraphicsApplication::sampleMain() {
    ...
    init_window(info);
    init_swapchain_extension_patched(info); // PATCHED
    init_device(info);
